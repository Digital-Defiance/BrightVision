...
- [x] 7. Tauri config resolver — _Requirements: REQ-001, REQ-003_ (depends: none)
  ...
  - [x] 7.2 In same file, add `fn validate_backend(backend: &str) -> Result<String, String>` checking `ALLOWED_BACKENDS` + `cfg!(target_os)`. Panic on malformed at init. (REQ-003.4)
    - verify: `cargo check -p brightvision-desktop`
...
- [x] 10. Frontend UI adaptation — _Requirements: REQ-004_ (depends: 9)
  ...
  - [x] 10.2 Add 2s timeout wrapper on IPC backend call — render `<Alert>Backend unavailable</Alert>` and disable controls on timeout. (REQ-004.4)
    - verify: `npx tsc --noEmit src/components/local-llm/LocalLlmPanel.tsx`
