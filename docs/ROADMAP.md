...
| **53** | **Done** | **Kiro-depth spec agent** — `spec_gen_agent.py`: repo map on spec jobs, `.cecli/STEERING.md` in generate prompts, read-only `/agent` explore pass, heavy-tier routing when model router on, auto **deepen** pass when `assess_spec_richness` fails (`BV_SPEC_GEN_AGENT` / `BV_SPEC_GEN_RICHNESS_GATE`, off when `BV_COMPACT_SPEC_GEN=1`). **Tests:** `test_spec_gen_agent.py`. |
